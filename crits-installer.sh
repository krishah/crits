...
cd ..
#chopshop
apt install autogen build-essential python-dev zlib1g-dev autoconf libtool
...
ln -s /usr/local/lib/python2.7/dist-packages/usr/lib/libyara.so /usr/lib
cd ../..
#snugglefish
git clone https://github.com/MITRECND/snugglefish
cd snugglefish
make all
...
cp ./tokumx.sh /etc/service/tokumx/run
cp ./tokumx-check.sh /etc/service/tokumx/check
export PATH=/opt/tokumx/bin:$PATH
